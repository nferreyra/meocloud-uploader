...
#Don't edit these...
API_REQUEST_TOKEN_URL="https://meocloud.pt/oauth/request_token"
API_USER_AUTH_URL="https://meocloud.pt/oauth/authorize"
API_ACCESS_TOKEN_URL="https://meocloud.pt/oauth/access_token"
API_CHUNKED_UPLOAD_URL="https://api-content.meocloud.pt/1/ChunkedUpload"
API_CHUNKED_UPLOAD_COMMIT_URL="https://api-content.meocloud.pt/1/CommitChunkedUpload"
API_UPLOAD_URL="https://api-content.meocloud.pt/1/Files"
API_DOWNLOAD_URL="https://api-content.meocloud.pt/1/Files"
API_DELETE_URL="https://publicapi.meocloud.pt/1/Fileops/Delete"
API_MOVE_URL="https://publicapi.meocloud.pt/1/Fileops/Move"
API_COPY_URL="https://publicapi.meocloud.pt/1/Fileops/Copy"
API_METADATA_URL="https://publicapi.meocloud.pt/1/Metadata"
API_INFO_URL="https://publicapi.meocloud.pt/1/Account/Info"
API_MKDIR_URL="https://publicapi.meocloud.pt/1/Fileops/CreateFolder"
API_SHARES_URL="https://publicapi.meocloud.pt/1/shares"
APP_CREATE_URL="https://meocloud.pt/my_apps"
...
function usage
{
    echo -e "MeoCloud Uploader v$VERSION"
    echo -e "Nuno Ferreira - nferreira@gmail.com\n"
    ...
    if grep -q "HTTP/1.1 400" "$RESPONSE_FILE"; then
        ...
        case $ERROR_MSG in
             *access?attempt?failed?because?this?app?is?not?configured?to?have*)
                echo -e "\nError: The Permission type/Access level configured doesn't match the MeoCloud App settings!\nPlease run \"$0 unlink\" and try again."
                exit 1
            ;;
        esac
...
function db_stat
{
    ...
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" "$API_METADATA_URL/$ACCESS_LEVEL/$(urlencode "$FILE")?oauth_consumer_key=$APPKEY&oauth_token=$OAUTH_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$APPSECRET%26$OAUTH_ACCESS_TOKEN_SECRET&oauth_callback=oob&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" 2> /dev/null
    check_http_response

    #Even if the file/dir has been deleted from MeoCloud we receive a 200 OK response
...
function db_upload_file
{
    ...
    if [[ $basefile_dst == "thumbs.db" || \
          ...
          $basefile_dst == "icon\r" || \
          $basefile_dst == ".meocloud" || \
          $basefile_dst == ".meocloud.attr" \
       ]]; then
...
function db_chunked_upload_file
{
    ...
    while (true); do
        ...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "upload_id=$UPLOAD_ID&oauth_consumer_key=$APPKEY&oauth_token=$OAUTH_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$APPSECRET%26$OAUTH_ACCESS_TOKEN_SECRET&oauth_callback=oob&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_CHUNKED_UPLOAD_COMMIT_URL/$ACCESS_LEVEL/$(urlencode "$FILE_DST")" 2> /dev/null
        check_http_response
...
#Returns the free space on MeoCloud in bytes
function db_free_quota
{
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$APPKEY&oauth_token=$OAUTH_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$APPSECRET%26$OAUTH_ACCESS_TOKEN_SECRET&oauth_callback=oob&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_INFO_URL" 2> /dev/null
    check_http_response
...
{
    print "MeoCloud Uploader v$VERSION\n\n"
    print " > Getting info... "
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o "$RESPONSE_FILE" --data "oauth_consumer_key=$APPKEY&oauth_token=$OAUTH_ACCESS_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$APPSECRET%26$OAUTH_ACCESS_TOKEN_SECRET&oauth_callback=oob&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_INFO_URL" 2> /dev/null
    check_http_response
...
function db_unlink
{
    echo -ne "Are you sure you want unlink this script from your MeoCloud account? [y/n]"
    read answer
    if [[ $answer == "y" ]]; then
        rm -fr "$CONFIG_FILE"
        echo -ne "DONE\n"
...
if [[ -f $CONFIG_FILE ]]; then
    ...
    #Back compatibility with previous MeoCloud Uploader versions
    if [[ $ACCESS_LEVEL == "" ]]; then
        ACCESS_LEVEL="meocloud"
    fi
...
#NEW SETUP...
    ...
    echo -ne " 2) Click on \"Create App\", then select \"MeoCloud API app\"\n"
    ...
    while (true); do
        ...
        echo -n " # Permission type, App folder or Full MeoCloud [a/f]: "
        read ACCESS_LEVEL
    ...
    #TOKEN REQUESTS
    echo -ne "\n > Token request... "
    $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o $RESPONSE_FILE --data "oauth_consumer_key=$APPKEY&oauth_signature_method=PLAINTEXT&oauth_signature=$APPSECRET%26&oauth_callback=oob&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_REQUEST_TOKEN_URL" 2> /dev/null
    check_http_response
    ...
    while (true); do
        ...
        #USER AUTH
        echo -ne "\n Please open the following URL in your browser, and allow MeoCloud Uploader\n"
        echo -ne " to access your MeoCloud folder:\n\n --> ${API_USER_AUTH_URL}?oauth_token=$OAUTH_TOKEN\n"
        ...
        $CURL_BIN $CURL_ACCEPT_CERTIFICATES -s --show-error --globoff -i -o $RESPONSE_FILE --data "oauth_consumer_key=$APPKEY&oauth_token=$OAUTH_TOKEN&oauth_signature_method=PLAINTEXT&oauth_signature=$APPSECRET%26$OAUTH_TOKEN_SECRET&oauth_verifier=$APPPIN&oauth_callback=oob&oauth_timestamp=$(utime)&oauth_nonce=$RANDOM" "$API_ACCESS_TOKEN_URL" 2> /dev/null
        check_http_response
